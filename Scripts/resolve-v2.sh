...
if $FULLHD; then
  ...
  ffmpeg -hwaccel cuda -i "$INPUT_FILE" -vf scale=1920:1080 -c:v av1_nvenc -preset p6 -cq 28 -c:a pcm_s16le "$FULLHD_FILE"
  #ffmpeg -hwaccel cuda -i VID_20180627_140325.mp4 -c:v av1_nvenc -preset p6 -cq 28 -c:a pcm_s16le VID_20180627_140325f.mp4
...
# Output file
OUTPUT_FILE="$DIR_NAME/$BASE_NAME.new.mp4"
...
ffmpeg -hwaccel cuda -i "$INPUT_FILE" -c:v av1_nvenc -preset p6 -cq 28 -c:a pcm_s16le "$OUTPUT_FILE"
